...
notes:
  ...
  - "2026-09-18 (worker): PR #85 open, gate green, branch `claude/can-this-be-resolved-of6kjq` (a harness-assigned branch, so the `[flow-0057]` PR title prefix is what the automation reads). CORRECTION TO THE NOTE OF 2026-09-16, which a reader should not take at face value: that note argues the average must be kept because sparse `0 7 * * 1-5` gives a 67.2h crit bound `which survives the 63h weekend gap`. 63h is the OTHER cron's gap (`0 9-18 * * 1-5`, Fri 18:00 to Mon 09:00). The sparse cron's weekend gap is Fri 07:00 to Mon 07:00 = 72h, which EXCEEDS 67.2h — so the sparse case was already crit every weekend too, just later in the day. The averaging protected less than the task believed. This does not change the fix, it strengthens it: one longest-gap rule clears both, and criterion 3 now pins the 72h figure so the wrong number cannot be reintroduced. RULE SHIPPED: crit at `maxGap + interval`, warn at `maxGap`; for an evenly spaced cron maxGap equals the average, so the bound is `interval * 2` to the digit and the three original boundary tests pass unchanged — the change is confined to clustered crons by construction rather than by special-casing. `maxGap * 2` was rejected: it would delay a clustered cron's alarm to 126h. NOT DONE HERE, deliberately: the reason string keeps its `cron interval ~Yh` prefix verbatim because `flightdeck/bin/watchdog.test.mjs` matches it and that file is outside this task's `touches`; if a future task widens the wording, that test moves with it. flow-0065 remains live and independent."
---
