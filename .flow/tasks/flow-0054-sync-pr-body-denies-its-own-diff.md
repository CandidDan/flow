...
branch: "flow/flow-0054-sync-pr-body-denies-its-own-diff"
pr: "https://github.com/CandidDan/flow/pull/97"
issue: ""
blocked_reason: ""
blocked_by: []
serves: ["G10"]
...
notes:
  ...
  - "2026-09-24 (worker): branch `flow/flow-0054-sync-pr-body-denies-its-own-diff` pushed, no PR yet. DONE: `_flow-sync.yml` now computes `CHANGED=\"$(git diff --cached --no-renames --name-status)\"` AFTER `git add -A`, with a do-not-move-it-back comment; `prContent` groups Added/Modified/Removed with counts and keeps the `version stamp only` sentence for the one true case (`.flow/VERSION` alone); a new `parseChanges` export parses `--name-status`. Tests appended to `project-template/.flow/bin/flow-sync.test.mjs` lift the shipped shell out of `_flow-sync.yml` by regex and run it against three git fixtures (all-additions, mixed A/M/D, stamp-only) — 21/21 pass locally. NEXT: run the full gate (build/lint/test/coverage), write the changelog entry, open the PR. DECISION TAKEN: `touches` declares `CHANGELOG.md` and the acceptance criteria name it, but the repo adopted the `changes/<task-id>.md` fragment convention in flow-0069 AFTER this task was written; touches-guard would fail a `changes/` file, so the entry goes in `CHANGELOG.md` under `## Unreleased` and the PR says so. NOT DONE: `project-template/.github/workflows/flow-sync.yml` is unmodified — the thin caller is a pure `uses:` caller and carries no file-list computation; a proving test asserts that rather than leaving the reader to check."
  - "2026-09-24 (worker): PR #97 open and marked ready for review; branch `flow/flow-0054-sync-pr-body-denies-its-own-diff`. All eight acceptance criteria met with a named proving test each (see the PR body). Gate green on the rebased tip: build 34 workflows, lint 86 .mjs, test 1090 pass / 0 fail, coverage 95.39% lines vs the 83.5 floor. `project-template/.github/workflows/flow-sync.yml` is deliberately UNMODIFIED — the ordering never lived in the thin caller, which is a pure `uses:` caller, and a test now proves that rather than leaving the reader to check. The changelog entry is a direct `CHANGELOG.md` edit because `touches` and the acceptance criteria both name that file; flow-0069's `changes/<task-id>.md` convention landed after this task was written and touches-guard would have failed a `changes/` file. A sweep of the other tasks still declaring `CHANGELOG.md` is the orchestrator's call. NEXT: human reviews and merges #97; flow-done will set this task to done. Nothing further for a worker unless a review check kicks back."
---
