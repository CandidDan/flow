...
notes:
  ...
  - "2026-09-22 (worker): CRITERION 7 WITHDRAWN — it could not be proven without re-creating the trap flow-0047 removed. It read \"the `## Unreleased` section carries an entry for this change that states no caller action is required\". flow-0047 deleted an assertion over that section's CONTENTS precisely because the section's purpose is to change: any test pinned to this change's entry passes now and goes red the moment a release folds `Unreleased` into a numbered section, which is the failure that left flow-0045 with a red gate and no in-scope fix. The surviving, correct assertion — every entry states its caller action — is a property of a RELEASED section and `release-stamp.test.mjs` already holds it. The changelog entry is still owed and is still written; it is a Scope item, where it already was, rather than a criterion that cannot honestly carry a proving test. Writing it as a criterion was an orchestrator miss against task-writer's own rule: if you cannot write a test for it, it is not a criterion yet."
