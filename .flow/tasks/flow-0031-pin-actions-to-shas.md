...
touches: [".github/workflows/_flow-compass.yml", ".github/workflows/_flow-done.yml", ".github/workflows/_flow-gates.yml", ".github/workflows/_flow-open-pr.yml", ".github/workflows/_flow-queue-runner.yml", ".github/workflows/_flow-recover.yml", ".github/workflows/_flow-review.yml", ".github/workflows/_flow-status.yml", ".github/workflows/_flow-sync.yml", ".github/workflows/_flow-triage.yml", ".github/workflows/ci.yml", ".github/workflows/flow-watchdog.yml", ".github/workflows/release-tag.yml", ".flow/bin/action-pins.test.mjs", ".flow/bin/workflow-prompt-paths.test.mjs"]
labels: [infra, security, supply-chain]
notes:
  - "2026-08-31: INTERACTION WITH flow-0029, recorded so it is not rediscovered the hard way. flow-0029 adds .github/workflows/flow-release-publish.yml, and this task's check scans the workflow DIRECTORY rather than a file list, so that new workflow is in scope for the check the moment it lands. flow-0029 has been noted to SHA-pin its actions as it writes them, which makes this a no-op. If it lands tag-pinned anyway and this task is worked afterwards, the file is outside these touches and the correct move is `blocked` with a one-line reason, not a silent edit — the orchestrator then adds the path. Deliberately NOT pre-declared in touches: flow-doctor flags that as an overlap with flow-0029 and it would stop the two tasks running in parallel, which is a real cost for a case the note prevents."
  ...
  - "2026-09-09: UNBLOCKED by the human in-session (Dan, responding to the handoff), who authorised widening touches to add .flow/bin/workflow-prompt-paths.test.mjs so flow-0018's TRIAGE_STRUCTURE_BEFORE constant can carry the pinned refs. Recorded here because a worker widening its own touches is otherwise drift; this one was directed. Resumed in_progress by the same session on branch claude/next-task-ulxjfc, PR #63."
---
