...
notes:
  - "2026-09-14: GATE GREEN, opening the PR. Branch flow/flow-0044-release-1-3-0 rebased onto main (bdb4ea7) and force-pushed; 3 commits. The only new work this session was the locator fix the orchestrator widened touches for: .flow/bin/triage-author-trust.test.mjs now finds the section that CONTAINS the flow-0036 entry (a small entries() helper + a find over all `## ` sections) instead of assuming `## Unreleased`. Every assertion in that file is byte-for-byte unchanged; only the test NAME lost its stale `under Unreleased` clause. Verified green on this tip: build 24 workflows, lint 80 .mjs, npm test 953 tests / 0 fail / 1 skip (the previously failing case is now ok 453 and it RUNS rather than skipping), coverage 94.68 percent lines vs the 83.5 floor, and node .flow/bin/release-guard.mjs exits 0 with 0 problems and 1 warning (alias rot, 305 commits; facts carry mainAhead 305 too, and rule 4 correctly stopped firing because the stamp now leads the newest release tag). Re-derived the enumeration independently rather than trusting the earlier note: git log v1.2.0..HEAD over the reusables and project-template gives 16 ids by subject, plus flow-0036 (b5cbfe3, subject does not name the id) and flow-0043 (merge 3049e00, touches project-template/.flow/bin/flow-doctor.mjs) = the 18 in the test's tombstone list; flow-0044 is excluded correctly because the stamp bump IS the release. ENVIRONMENT SCAR worth knowing: the remote URL's embedded PAT is 403 for pushes (no Contents write) - pushing works only via the runner's DEFAULT_WORKFLOW_TOKEN. NEXT ACTION: none for a worker. The PR is open and the three human tag steps are at the end of its description."
