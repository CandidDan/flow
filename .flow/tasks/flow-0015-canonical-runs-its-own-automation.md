...
notes:
  ...
  - "2026-08-21: CI ROUNDS AFTER THE PR OPENED — two red rounds, both real, both fixed and pushed (51c3106, then the fixture rebuild). Round 1: a smoke test pinned this task to status 'in_progress'; flow-status flips it to 'in_review' the moment the PR opens, so the assertion was invalidated by the very automation this task wires up. Now asserts the row exists with SOME lifecycle value. Round 2: EACCES copying a fixture that already held git's mode-0444 loose objects — invisible under this container's root shell, red for the CI runner's user. Each fixture is now built from REPO rather than by copying another fixture, and the PR #13 regression fixture omits the adapter by absolute path so it no longer also removes the template's flow-state.mjs. GENERAL LESSON FOR FUTURE SESSIONS, worth not re-learning: never assert a task's lifecycle value in a test, and remember the worker runs as root while CI does not."
  - "2026-08-21: HANDOFF. Branch flow/flow-0015-canonical-runs-its-own-automation, draft PR #24. GENUINELY DONE: all five callers written (flow-open-pr, flow-recover, flow-triage, flow-review, flow-queue-runner), each @main with job-level permissions matching its reusable and secrets: inherit; queue-runner is workflow_dispatch-only with the omission documented in-file and asserted by two tests; the flow-sync absence is explained in flow-open-pr.yml. .flow/bin/flow-state.mjs added as a thin adapter, enabled by extracting the template CLI into an exported runStateCli({ repoRoot, argv, out }) and exporting readTasksFromOrigin/readPrs. adapters.test.mjs +12 tests, flow-state.test.mjs +10. Full gate green: build 19 workflows, lint 45 mjs, 423 tests (422 pass / 1 pre-existing skip), coverage 95.75 vs floor 83.5. touches-guard decision=enforced outside=0. Suite re-run with node_modules removed: 22 pass / 4 visibly skipped / 0 fail, so the no-install flow-tooling job is safe. NOT DONE, and not this task: nothing is enabled — vars.FLOW_AI and CLAUDE_CODE_OAUTH_TOKEN remain unset, so four of the five callers are dormant. flow-open-pr is the exception and goes live on merge, by design (it invokes no agent, so it carries no FLOW_AI gate). DECISIONS A FRESH SESSION SHOULD NOT RE-LITIGATE: the @main pin is deliberate, do not correct it to @v1; queue-runner gets no cron for cadence reasons, not permissions; the template refactor is code motion only and the template CLI still resolves its own root. AWAITING HUMAN: ratification of the touches widening (see the note above), and a separate deliberate decision about enabling FLOW_AI here. NEXT ACTION: none for this task beyond review/merge; flow-done owns the done transition."
