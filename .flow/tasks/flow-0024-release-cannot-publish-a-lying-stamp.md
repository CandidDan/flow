...
notes:
  ...
  - "2026-08-20: PR #19 open, gate green (build/lint/test/coverage 90.08% vs floor 83.5, flow-doctor clean). MERGING THIS TURNS release-tag.yml RED, and that is the guard working, not a defect: v1.1.1 is history and permanently stamped 1.1.0, and the guard follows the NEWEST release tag. The repair is a human cutting v1.2.0 at main (whose VERSION already reads 1.2.0); it then becomes the highest release tag, agrees with its tree, and the edge alias moves again. The guard never retags anything itself. A test (`the documented repair clears it`) asserts that promise holds, so do not 'fix' the red by weakening the check."
  - "2026-08-20: the open question in this task (alias-rot warning in flow-doctor vs release-tag.yml only) was decided as release-tag.yml ONLY. flow-doctor.mjs is not in this task's touches and is claimed by flow-0017 and again by flow-0022; reaching for it would be exactly the silent scope drift touches exists to stop. checkRelease() is pure and exported, so moving the warning later is an import and a call, not a rewrite. Worth a follow-up task if the warning proves too easy to ignore — the escalation this task records is a scheduled report, never a stricter release gate."
  - "2026-08-20: scar for anything new under .flow/bin/. The flow-tooling job in _flow-gates.yml runs `node --test .flow/bin/*.test.mjs` with NO `npm ci` in front of it, so any import of yaml or c8 dies before a single test runs. check-workflows.test.mjs and config.test.mjs handle it with a conditional skip (17 tests skip there); this task went dependency-free instead, so its wiring assertions run in both jobs. Reproduce the condition locally with node_modules moved aside — the normal local run cannot see this failure."
  - "2026-08-20: correction to the misdiagnosis note above, found while writing the test. git DOES peel tag objects for `<rev>:<path>` and for rev-list ranges, so the stamp and the distances read correctly either way. What does not peel is a sha held up against a commit listing: a tag-object sha is not on main and never will be, so whoever compares them concludes the tag points off the branch. That is the actual mechanism of the 2026-08-19 misread, and it is what the annotated-tag test now pins. The `<tag>^{commit}` rule stands; the reason for it is narrower than stated."
---
