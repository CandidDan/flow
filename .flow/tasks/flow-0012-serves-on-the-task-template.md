...
notes:
  ...
  - "2026-08-21: HANDOFF. Branch flow/flow-0012-serves-on-the-task-template, draft PR #23 (https://github.com/CandidDan/flow/pull/23). DONE and verified: serves on project-template/.flow/tasks/_TEMPLATE.md between blocked_reason and touches with the four-rule comment block; task-writer woven in six places (step 1 required reading, new step 5 setting serves, readiness test, pre-flight item 3 + doctor-as-backstop, triage Propose and close lanes, Don't list); 21 proving tests in .flow/bin/serves-template.test.mjs. Full gate green locally: build 14 workflows, lint 45 .mjs, 421 tests (420 pass 1 skip), coverage lines 93.88 vs floor 83.5, flow-doctor healthy, touches-guard decision=enforced reason=in-scope outside=0 (first real exercise of flow-0008 assertion). DECISIONS a fresh session should NOT re-litigate: (1) tests were written despite this task's Notes warning against prose-regex tests — they are section-scoped and concept-level, plus three genuine integration tests running the shipped frontmatter through flow-0010 flow-doctor including a negative control; flow-0011 set the precedent. (2) the test lives in canonical .flow/bin/ because touches-guard excludes .flow/** and that is where flow-0011 put its equivalent. NEXT ACTION: none for a worker — the PR is open and flow-status owns in_review. Human owes ratification of the touches widening and a ruling on (1); both are stated in the PR body."
---
