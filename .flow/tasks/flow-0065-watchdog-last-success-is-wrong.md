...
notes:
  - "2026-09-21 (orchestrator): THE WEEKEND MYSTERY IS SOLVED, AND TWO OF MY NOTES BELOW ARE WRONG. flow-0057 was FIXED while I was watching it: PR #85 merged 2026-09-18T11:18Z and canonical commit 1e24d6c marks flow-0057 done. The new rule is visible in the issue text itself \u2014 Nudge#292, filed by Monday\u0027s 08:10 sweep, reads `last success 1.1h ago, cron interval ~0.5h, longest scheduled gap ~0.5h \u2014 past that gap plus one interval of slack`, where Thursday\u0027s #289 read `last success 230.1h ago, cron interval ~3.4h`. Every null I recorded is that fix working: Sat, Sun and Mon sweeps correctly declined to alarm on `0 9-18 * * 1-5` gaps of 14h, 38h and 62h, because the longest legitimate scheduled gap now bounds the check. MY ERROR WAS METHODOLOGICAL, not arithmetic: I treated a behaviour change as evidence about the defect without checking whether the defect had been FIXED. One `git log` on canonical would have answered it on Saturday. The notes dated 2026-09-19 (\u0027AN OBSERVATION I CANNOT EXPLAIN... cuts against flow-0057\u0027) and 2026-09-20 (\u0027Something changed between Thursday and Saturday\u0027) are superseded: nothing cut against flow-0057, and the thing that changed was its own fix landing."
  - "2026-09-21 (orchestrator): THIS TASK SURVIVES THAT FIX INTACT, verified rather than assumed. PR #85\u0027s diff touches only `flightdeck/bin/liveness.mjs` and `liveness.test.mjs` \u2014 226 lines across two files, nothing else. `flightdeck/bin/watchdog.mjs:290` on current `main` still reads `runs?status=success&per_page=1`, and :297 still has the same shape. So flow-0057 fixed the THRESHOLD and left the LOOKUP untouched, which is exactly the independence the 2026-09-17 note claimed. The core evidence is unaffected: #289 stated a last-success of 2026-09-07T18:04:59 when the true newest success was 2026-09-16T18:05:13, wrong by nine days, and no threshold change can repair a wrong datum. One consequence for whoever works this: the wrong figure has not reproduced since 2026-09-17, and with the false positives gone there are now far fewer issues being filed, so waiting for another live instance is no longer a viable strategy \u2014 reproduce it from a fixture instead, as the acceptance criteria already require."
  - "2026-09-20 (orchestrator): SECOND CONSECUTIVE NULL, at a LARGER gap \u2014 sharpening the open question rather than adding a new one. The Sunday ~08:08 sweep also filed nothing for `flow-queue-runner`. Last success is still run 666 at 2026-09-18T18:05:09Z, so the gap at that sweep was ~38h, against the ~6.7h bound. The pattern across five sweeps: Wed 2026-09-16 FILED (#278, 14.1h), Thu 2026-09-17 FILED (#289, 14.1h real / 230.1h claimed), Fri 2026-09-18 no file (masked by #289 still open), Sat 2026-09-19 CLOSED #289 and did not file (14.1h), Sun 2026-09-20 did not file (~38h). So it filed twice at 14h and then stopped filing even as the gap grew — which is not what a pure threshold defect does, and not what the masking explanation covers either, since #289 was closed by Saturday. Something changed between Thursday and Saturday. The weekend is the obvious confound (`0 9-18 * * 1-5` does not fire Sat/Sun, so `cronIntervalHours` over a 28-day window may be computing something different), but that is a guess. MONDAY 2026-09-21\u0027s weekday sweep is the discriminating observation: if it files with a correct figure, flow-0065 is intermittent and flow-0057 holds; if it files with a wrong figure, flow-0065 reproduces; if it does not file at all, both tasks need re-examining against the code."
