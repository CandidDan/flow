...
branch: "flow/flow-0058-init-adopts-from-v1"
pr: "https://github.com/CandidDan/flow/pull/100"
issue: ""
blocked_reason: ""
blocked_by: []
serves: ["maintenance"]
...
notes:
  - "2026-09-24 (worker): HANDOFF. Branch `flow/flow-0058-init-adopts-from-v1`, pushed; PR #100 (opened as a draft by flow-open-pr, body carries the full criterion-to-test mapping). DONE, all seven criteria: `DEFAULT_CANONICAL_REF = \"v1\"` is DELETED, replaced by `refFromVersion` / `versionStamps` / `resolveDefaultRef` in `project-template/.flow/bin/flow-init.mjs`. The default ref is now the major of the same `VERSION` file `buildPlan` copies to the target's `.flow/VERSION`, so pins and stamp agree by construction. Stamp priority: `<--from>/VERSION` (the authority, because it IS the file that becomes the stamp), then canonical root `VERSION` for the clone path (gated on the same `project-template/` test `buildPlan` uses), then `.flow/VERSION` for the copy that lives in an adopted repo. No stamp answers -> no default: a named `canonical.ref is required` validation failure, never a guess. `USAGE` became `renderUsage({ref,stamp})` so `--help` and the run render one value. 9 new/updated tests in `flow-init.test.mjs`; every expectation derived from a VERSION stamp, never a literal. Gate green after rebasing on main at 89cfd21: build 34 workflows, lint 87 .mjs, test 1111 (1110 pass / 0 fail / 1 pre-existing skip), coverage 95.45% lines vs floor 83.5. touches-guard locally: decision=enforced reason=in-scope checked=3 globs=3 outside=0. Proved by mutation: restoring a fixed `v1` fails 9 tests, including every criterion test. NEXT ACTION: `gh pr ready 100`, then stop — the qa/security/code-review checks run on the PR, not in session. NOT done and deliberately out of scope per the task's Scope: the ten published callers and `_flow-sync.yml` (flow-0056 owns both), `DEFAULT_CANONICAL_REPO`, a `repin` call in `flow-sync`, and any retro-fix of repos already born at `@v1` — `changes/flow-0058.md` names the remedy (re-run with an explicit `--canonical-ref`, or let flow-sync deliver the @v2 callers)."
