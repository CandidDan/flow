...
notes:
  ...
  - "2026-08-27: THIRD OFFENDER, for the orchestrator — the new check found .github/workflows/_flow-compass.yml citing .claude/skills/flow-compass/SKILL.md, the same root cause as the two this task fixes. It is outside flow-0018's declared touches, and Scope forbids changing another reusable, so it is NOT fixed here. It is recorded in KNOWN_UNFIXED in .flow/bin/workflow-prompt-paths.test.mjs, printed on every failure, and the check fails if the entry ever stops describing reality — so the register cannot outlive the bug. Follow-on task needed: give that prompt the same both-locations wording _flow-triage.yml now carries (the file exists at project-template/.claude/skills/flow-compass/SKILL.md); touches would be [.github/workflows/_flow-compass.yml, .flow/bin/workflow-prompt-paths.test.mjs] to drop the register entry in the same change."
  - "2026-08-27: second follow-on, smaller — the task spec called for a .claude/agents/** exclusion as forward cover 'until flow-0007 merges'. flow-0007 is now done and no workflow cites an agent path, so per that note removing the agent-definitions exclusion is a one-line change; left in place here because removing it was not this task's call to make."
