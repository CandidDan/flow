...
notes:
  ...
  - "2026-09-24 (worker): BUILD COMPLETE, gate green, branch `flow/flow-0063-intent-store-and-its-validation` pushed (rebased on main at c3bebb1). DONE: `docs/adr/0007-intent-layer.md` (246 lines — the four-slice sequence, the plane decision, merge-as-event, `source:`, Outcome, plus five named decisions the criteria ask for: evidence-linkage, validation-contract deferral, grandfathering forward-only, teeth none-in-this-slice, triage-collision deferred, each with its reason and each under its own `###` heading so the proving tests can anchor on them); `.flow/intents/_TEMPLATE.md` and `project-template/.flow/intents/_TEMPLATE.md`, byte-identical (a test asserts that, canonical-only) with `id`/`title`/`status`/`created`/`source`/`approved_by`/`approved_at`/`evidence: []` and an Outcome section carrying the external-observable-change definition and the 'a dashboard exists' non-example; `flow-doctor` gains `INTENT_REQUIRED`, `evidenceShape()` and `intentFindings()`, wired into `runDoctor` after the vision block; 18 new tests. `parseTask` was refactored onto two new shared helpers (`splitFrontmatter`, `scalarReader`) so the task reader and the intent reader cannot disagree about what 'parses' means — no behaviour change, existing tests unchanged. Gate: build 34 workflows, lint 87 .mjs, test 1129 (1128 pass / 0 fail / 1 pre-existing skip), coverage 95.52% lines vs floor 83.5. `node .flow/bin/flow-doctor.mjs` on canonical's real store exits 0 with only pre-existing warnings. NEXT ACTION: open the PR titled `[flow-0063] Give G11 somewhere to live...`, then `gh pr ready`."
  - "2026-09-24 (worker): THINGS THE HUMAN MAY WANT TO OVERRULE AT APPROVAL. (1) The test fixture helper now creates an EMPTY `.flow/intents/` by default, with `intents: null` for the absent-store case — mirroring how `vision` already defaults to a real VISION.md. Without it the new adoption warning would have fired in every unrelated warning-count assertion in the file. (2) TWO FREE CHECKS DELIBERATELY NOT ADDED, flagged per the task's own note rather than written: `id` is not checked against the filename, and `status` is not checked against the documented `proposed|approved|superseded` vocabulary. Both are two lines; both are teeth this slice has no budget for. ADR-0007 records them under 'What was left unchecked, and would have been free' so adding them later is a decision rather than a patch. (3) The ADR fixes the file-naming convention flow-0072 asked for: `.flow/intents/<slug>.md` with `id` equal to the slug. flow-0072's dependency on this task's template shape is now resolvable. (4) Three proving tests read canonical-only paths (`docs/adr/0007-intent-layer.md`, canonical's own `.flow/intents/_TEMPLATE.md`) from a test file that ships to adopting repos, so they are guarded by an exact identity check — 'the tree three levels up holds THIS directory at project-template/.flow/bin' — not a bare existsSync. They RUN here (confirmed: tests 846-848 pass, not skip) and skip everywhere else. The alternative was a new test file in canonical's `.flow/bin/`, which is outside this task's `touches`."
  - "2026-09-24 (worker): PR #105 (https://github.com/CandidDan/flow/pull/105) marked ready for review; `flow-open-pr` had already opened it as a draft on the first branch push, so the worker edited its title/body and ran `gh pr ready` rather than `gh pr create`. Body carries the full criterion-to-test mapping for all 13 acceptance criteria and the five decisions flagged for the human. `flow-status` moves this task to `in_review` on the ready_for_review event; nothing further for the worker. Gate at the reviewed commit: build 34 workflows, lint 87 .mjs, test 1129 (1128 pass / 0 fail / 1 pre-existing skip), coverage 95.52% lines vs floor 83.5."
---
