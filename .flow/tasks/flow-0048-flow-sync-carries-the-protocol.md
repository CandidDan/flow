...
created: "2026-09-15"
started: "2026-09-24T03:07:07Z"
branch: "flow/flow-0048-sync-carries-the-protocol"
pr: "https://github.com/CandidDan/flow/pull/99"
issue: ""
blocked_reason: ""
blocked_by: []
serves: ["maintenance"]   # re-anchored 2026-09-23 from retired G4 (decision D1)
touches: [".github/workflows/_flow-sync.yml", ".flow/bin/sync-surface.test.mjs", "changes/flow-0048.md"]
labels: [infra, fleet, sync]
notes:
  - "2026-09-24 (worker): `touches` corrected, NARROWED not widened — `CHANGELOG.md` replaced by `changes/flow-0048.md`. This task was written 2026-09-15; the `changes/` directory landed 2026-09-23 (flow-0069) and the protocol now states the rule outright: a task's changelog entry goes in `changes/<task-id>.md`, a task never edits `CHANGELOG.md` directly, and `changes/<task-id>.md` is what goes in `touches` — never `CHANGELOG.md`. Acceptance criterion 6 asks that the entry state the caller action; it is stated, in `changes/flow-0048.md`. Three reasons this is a rule being applied rather than a decision being taken: (1) the protocol and `changes/README.md` both mandate the exact declaration, leaving nothing to choose; (2) the new path is narrower — no other task can collide on a file named after this one, whereas `CHANGELOG.md` is the serialisation point flow-0069 existed to remove; (3) `touches-guard` judges the diff against this list, so leaving the stale declaration in place would have failed the gate on a correctly-placed fragment. Flagged for the orchestrator rather than assumed silently."
  - "2026-09-24 (worker): PR #99 marked ready for review, body carries the full criterion-to-test mapping. All six acceptance criteria met; gate green after rebasing on main at d3173ca (build 34 workflows, lint 87 .mjs, test 1092 with 0 failures, coverage 95.37% lines vs floor 83.5). `touches-guard` locally: decision=enforced reason=in-scope checked=2 globs=3 outside=0. `flow-status` will move this task to `in_review` on the ready_for_review event; nothing further for the worker. Three things the human may want to weigh, all stated in the PR body: the `touches` correction recorded above, the merge order against PR #99's neighbour PR #97 (flow-0054, which edits the same workflow one line further down), and the `::warning` posture chosen for a `canonical_ref` pinned before the protocol split rather than failing those syncs."
  - "2026-09-24 (worker): HANDOFF. Branch `flow/flow-0048-sync-carries-the-protocol`, pushed. DONE: `_flow-sync.yml` copies `$CANON_TPL/.flow/PROTOCOL.md` (guarded, because a caller pinned at v1.1.1 or earlier has no such file and a bare `cp` under `set -e` would abort the whole sync — it warns naming the ref instead); the header's `What it syncs` inventory updated and its now-false `the CLAUDE.md protocol block stays a manual adopt` sentence removed; AGENTS.md/CLAUDE.md exclusion stated in a comment naming the `## Project notes` reason; `CANONICAL_REF` exported via GITHUB_ENV so the warning can name the pin; `.flow/bin/sync-surface.test.mjs` added (11 tests) asserting the surface by parsing the workflow AND by running its real extracted copy region against fixture repos; `changes/flow-0048.md` written. Gate green locally: build 34 workflows, lint 87 .mjs, test 1092/0 fail, coverage 95.37% lines vs floor 83.5. Criterion 4 demonstrated by mutation: with the `cp` line deleted from the real file, 7 of 11 tests fail. NEXT ACTION: rebase on main, re-run the gate, open the PR titled `[flow-0048] Make flow-sync carry .flow/PROTOCOL.md, so a protocol fix can reach the fleet at all`, then `gh pr ready`. NOT done and deliberately out of scope: nothing in `flow-sync.mjs`'s version-comparison brain, and no merge strategy for AGENTS.md/CLAUDE.md (that needs a merge preserving per-project notes, not a copy — a separate task if anyone wants it)."
  - "2026-09-24 (worker): OVERLAP WITH AN OPEN PR, for whoever merges. flow-0054 (`in_review`, PR #97) also edits `.github/workflows/_flow-sync.yml`. The claim was legitimate — `pick-task` blocks a `ready` task only against `in_progress`, and flow-0054 is past that — but the two diffs are adjacent in the same file: flow-0048 inserts the protocol copy immediately BEFORE `printf ... > .flow/VERSION`, and flow-0054 adds its comment and moves `CHANGED=` immediately AFTER it. Whichever merges second rebases; expect at most a trivial context conflict at that one line, and no semantic conflict — flow-0054 reshapes the variable the PR body is built from, flow-0048 adds a file to the surface the diff contains. flow-0054 also edits `CHANGELOG.md`, which flow-0048 now does not, so that half cannot collide at all."
