...
notes:
  - "2026-09-10: the count note below (49 / 28 / 11) is WRONG and the merged ADR carries 46 / 19 / 10 instead -- left here rather than rewritten, because being caught by the gate is the point. code-review found both halves of one mistake: `git grep 'CandidDan/flow'` is an unanchored substring that also matches `CandidDan/flow-protocol` and `CandidDan/flow-plugin` (three files name only those, so 49 -> 46), and GitHub parses `uses:` inside .github/workflows/*.yml and NOWHERE else, so seven runbooks, an HTML map and two .test.mjs fixtures are prose, not references (28 -> 19, of which 10 not 11 are template callers -- the 11th was project-template/.flow/bin/flow-init.test.mjs, a canonical-only fixture never shipped). The deeper defect was that the test re-derived the same flawed number it asserted against: self-confirming, not pinned to the tree. Fixed in c7b8500, which anchors the reference, scopes `uses:` to workflow files, and adds a tripwire so loosening the anchoring fails two tests instead of passing quietly. Lesson for the next counting task: a measurement that agrees with itself has proven nothing."
