...
notes:
  - "2026-09-15: PR OPEN — https://github.com/CandidDan/flow/pull/75, non-draft (so `flow-status` owns the `in_review` transition on the `opened` event; no `gh pr ready` was needed and none was run). Branch `flow/flow-0047-unreleased-exists`, rebased onto main and re-gated after the rebase. Work is complete against all five criteria; the checklist with the proving test named for each is in the PR description. ENVIRONMENT SCAR, worth recording because it cost two failed attempts: pushes need $DEFAULT_WORKFLOW_TOKEN (the remote URL's embedded PAT is 403, no Contents write) but PR CREATION needs the opposite token — $DEFAULT_WORKFLOW_TOKEN is an Actions token and GitHub refuses `createPullRequest` from one ('GitHub Actions is not permitted to create or approve pull requests'), while the default $GH_TOKEN creates it fine. So: push with DEFAULT_WORKFLOW_TOKEN, `gh pr create` with the default token. NEXT ACTION: none for a worker — the human reviews and merges. flow-0045's changelog blocker clears on merge; its FLOW_PAT credential blocker does not."
  - "2026-09-15: GATE GREEN, branch `flow/flow-0047-unreleased-exists` pushed (1 commit). GENUINELY DONE: the `## Unreleased` case in `.flow/bin/release-stamp.test.mjs` now asserts only that the section EXISTS (via a shared `assertUnreleasedPresent(text, path)` helper so the same check runs over the real file and over fixtures), and three new cases prove the rest — a populated fixture passes, an empty fixture passes, and a mutation that deletes `## Unreleased` from the REAL changelog text fails with a message naming the loss and the path. The other four cases (two stamps, released-section exists/non-empty, every-entry-names-a-caller-action) and both tombstones are byte-for-byte unchanged — no shared helper needed touching, so criterion 4 needed no diff explanation. CHANGELOG.md carries this change's own entry under `## Unreleased`, which is criterion 1's live proof: the gate is green WITH an entry present. Verified on the branch tip: build 24 workflows, lint 80 .mjs, npm test 958 tests / 957 pass / 0 fail / 1 skip (new cases are ok 376-379), coverage 94.68% lines vs the 83.5 floor. ENVIRONMENT SCAR (same as flow-0044's): the remote URL's embedded PAT is 403 on push; only $DEFAULT_WORKFLOW_TOKEN works, so `git remote set-url origin https://x-access-token:$DEFAULT_WORKFLOW_TOKEN@github.com/CandidDan/flow.git` before any push. NEXT ACTION: open the PR titled `[flow-0047] Stop the release-stamp test forbidding the thing `## Unreleased` exists for` with the acceptance checklist, then `gh pr ready`. Nothing to redo — both files are on the branch and green."
