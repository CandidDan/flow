...
notes:
  ...
  - "2026-09-25 (worker): DONE — PR #110 is open and marked ready for review. Branch `flow/flow-0078-flow-init-test-passes-downstream`, rebased on main, two commits. Fixture fix + `.flow/bin/flow-init-downstream.test.mjs` + `changes/flow-0078.md`. Gate re-run after the rebase: build 34 workflows, lint 89 .mjs, test 1173/1172 pass 0 fail (1 pre-existing skip), coverage 95.67% lines vs floor 83.5. Negative control verified by hand: restoring the pre-fix `flow-init.test.mjs` turns the harness red on exactly the two Nudge failures (tests 1 and 14). Observed cause recorded in the PR body — Nudge's board has NO `const REPO` line at all, so prepareBoard's anchored regex had nothing to rewrite; `flow-init.mjs` untouched. Known and out of scope: `.gitattributes`/`.gitignore` are still sourced from TEMPLATE_ROOT (true in every flow-init'd repo), and a repo still stamped 1.x would additionally fail the `--help`/`@v1` assertion. NEXT: human review + merge, then cut a release so Nudge#297 can sync."
---
