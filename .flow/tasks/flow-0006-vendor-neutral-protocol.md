...
notes:
  - "2026-08-19: PR #15 open on branch claude/flow-006-sbtit3; all five gate commands green (build 14 workflows, lint 34 .mjs, 245 pass/0 fail/1 skipped, coverage 88.3 vs floor 83.5). All six acceptance criteria met and named to a proving test in .flow/bin/protocol-portability.test.mjs. Decisions a fresh session should NOT re-litigate: (1) the proving test lives in .flow/bin/, outside this task's touches — touches-guard excludes all of .flow/ by design and flow-0011 set the precedent; a test under project-template/.flow/bin/ would ship downstream and fail in every adopting repo. (2) Criterion 1 is pinned by SHA-256 digests of each pre-move section rather than a stored copy, because a stored copy would itself violate criterion 3 and git history is not guaranteed in a shallow CI checkout; an intentional future protocol edit is EXPECTED to fail that test and must recompute the digest in the same commit. (3) The preamble of PROTOCOL.md is the only rewritten prose — the sentence about being loaded automatically every session is false once the file is not CLAUDE.md; every ## section moved byte-for-byte. (4) Criterion 4 was verified live, not asserted: Claude Code's InstructionsLoaded hook reported load_reason=include with parent CLAUDE.md, and a behavioural question answered correctly with the import present and not at all with it removed. That check is committed as an opt-in test (FLOW_LIVE_AGENT_CHECK=1), skipped in CI which has no credentials. (5) No context saving is claimed — Claude Code expands the import at launch. "
  - "2026-08-19: found while building, deliberately NOT fixed because all three are outside this task's touches and would fail touches-guard: project-template/README.md line 14 still calls CLAUDE.md 'The protocol'; project-template/.flow/bin/flow-sync.mjs:14 and pick-task.mjs:4 point readers at CLAUDE.md 'Hard rules' in comments (harmless — criterion 5 is about code, and the test is comment-aware — but stale); and the Response style section still says Claude Code sessions auto-load this file, left verbatim because criterion 1 requires the sections be identical. Candidates for one small follow-up task, none blocking. "
