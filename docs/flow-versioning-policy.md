...
| `vMAJOR` (e.g. `v1`) | `flow-release-publish.yml`'s `mirror-alias` job | **Mirrors canonical's `vMAJOR`.** Fires on the `push` event for that tag — which is step 6 above — and force-updates the release repo's alias to the `vX.Y.Z` the stamp at that commit names. |
...
publish first (step 4's tag drives it), move the alias second. If the tag-push event is ever
...
- **2026-09-15** — the release repo gained the `vMAJOR` alias the fleet actually pins. Until then
  ...
  `@v1` there would have resolved to nothing. Mirrored from canonical's `v1` rather than advanced
  on publish, so the canary survives the split.
